...
const ThreadGroup = ({ subject, emails, leadId, defaultExpanded }) => {
  ...
  const sortedMessages = [...emails].sort((a, b) => new Date(a.received_at) - new Date(b.received_at));
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg mb-4 overflow-hidden shadow-sm">
      ...
      >
        <div className="flex-1 min-w-0 pr-4">
          <div className="flex items-baseline gap-2">
            <h3 className="font-bold text-gray-900 truncate">{mostRecent.subject || subject || '(No Subject)'}</h3>
            {emails.length > 1 && (
              <span className="text-gray-500 text-sm font-medium">({emails.length})</span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          <div className="text-sm text-gray-500">
            {mostRecent.received_at && formatDistanceToNow(new Date(mostRecent.received_at), { addSuffix: true })}
          </div>
      ...
      {expanded && (
        <div className="p-2 border-t border-gray-200 divide-y divide-gray-100">
...
const EmailThread = ({ emails, leadId }) => {
  ...
  const threads = Object.entries(grouped).map(([key, groupEmails]) => {
    const sorted = [...groupEmails].sort((a, b) => new Date(b.received_at) - new Date(a.received_at));
    return {
      key,
      subject: sorted[0].subject,
      emails: sorted,
      latestDate: new Date(sorted[0].received_at)
    };
