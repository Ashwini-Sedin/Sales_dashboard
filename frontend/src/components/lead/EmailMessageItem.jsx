    ...
    <div className="py-4 px-2">
      <div className="flex justify-between items-start mb-2">
        <div>
          ...
          <span className="text-gray-500 text-sm">&lt;{message.sender_email}&gt;</span>
        </div>
        <div className="text-sm text-gray-500 whitespace-nowrap">
          {message.received_at && format(new Date(message.received_at), 'dd MMM yyyy, hh:mm a')}
        </div>
      </div>
      
      <div className="mb-4">
        <span className="text-sm text-gray-500 mr-2">To:</span>
        <span className="text-sm text-gray-700">{formatRecipients(message.recipients)}</span>
      </div>

      <div className="bg-white rounded border border-gray-100 mb-3 overflow-hidden">
        <iframe
      ...
      {message.has_attachments && attachments.length > 0 && (
        <div className="mt-3">
          <div className="text-xs text-gray-500 mb-1 font-medium">Attachments:</div>
