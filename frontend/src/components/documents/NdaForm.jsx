    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-[640px] max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black text-gray-900">Generate NDA</h2>
            <p className="text-gray-500 text-sm font-medium">Standard legal non-disclosure agreement</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
              ...
              {typeOptions.map(opt => (
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 transition-colors ${
                    ndaType === opt.id ? 'bg-blue-600 text-white' : 'bg-gray-50 text-gray-400 group-hover:bg-gray-100'
                  }`}>
                    {opt.icon}
                  </div>
                  <h4 className={`font-bold text-sm ${ndaType === opt.id ? 'text-blue-900' : 'text-gray-900'}`}>{opt.title}</h4>
                  <p className="text-[10px] text-gray-500 font-medium leading-relaxed mt-1">{opt.desc}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <label className="block">
                <span className="text-xs font-bold text-gray-700 ml-1">Company Signatory</span>
                <select
                  required
                  value={formData.company_signatory_user_id}
                  onChange={(e) => handleInputChange('company_signatory_user_id', e.target.value)}
                  className="mt-1 block w-full rounded-2xl border-gray-100 bg-gray-50 focus:bg-white focus:ring-blue-500 transition-all text-sm font-medium p-3"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="block">
                  <span className="text-xs font-bold text-gray-700 ml-1">Client Legal Name</span>
                  <input
                    type="text"
                    required
                    ...
                    className="mt-1 block w-full rounded-2xl border-gray-100 bg-gray-50 focus:bg-white focus:ring-blue-500 transition-all text-sm p-3"
                  />
                </label>
                <label className="block">
                  <span className="text-xs font-bold text-gray-700 ml-1">Client Signatory Name</span>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="block">
                  <span className="text-xs font-bold text-gray-700 ml-1">Client Signatory Title</span>
                  <input
                    type="text"
                    required
                ...
                <label className="block">
                  <span className="text-xs font-bold text-gray-700 ml-1">Client Address</span>
                  <textarea
                    required
                    rows="1"
            ...
            {ndaType === 'multilateral' && (
              <div className="pt-4 space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-bold text-gray-900">Additional Parties</h4>
                ...
                {formData.additional_parties.map((party, idx) => (
                  <div key={idx} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-3 relative animate-in slide-in-from-right-2">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <label className="block">
                <span className="text-xs font-bold text-gray-700 ml-1">Effective Date</span>
                <input
                  type="date"
                  required
              ...
              <label className="block">
                <span className="text-xs font-bold text-gray-700 ml-1">Term (Years)</span>
                <input
                  type="number"
                  min="1"
                  max="10"
            ...
            <label className="block">
              <span className="text-xs font-bold text-gray-700 ml-1">Purpose of Disclosure</span>
              <textarea
                required
                rows="3"
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[10px] text-gray-500 font-bold mt-1 uppercase tracking-widest">Default clauses are pre-selected. Customize as needed.</p>
            </div>
                  ...
                  <div key={clause.id} className="bg-white">
                    ...
                    {isExpanded && (
                      <div className="px-14 pb-4 animate-in slide-in-from-top-2">
                        <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 text-[11px] text-gray-600 leading-relaxed max-h-32 overflow-y-auto">
                          {clause.clause_text}
                        </div>
                      </div>
                    )}
          ...
          <div className="border-t border-gray-100 pt-8">
            ...
            {isPreviewExpanded && (
              <div className="mt-4 p-8 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 space-y-6 animate-in slide-in-from-bottom-4">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-gray-500 font-medium">This agreement is entered into as of <b>{formData.effective_date}</b></p>
                </div>

                <div className="grid grid-cols-2 gap-8 text-center">
                  <div className="p-4 bg-white rounded-2xl shadow-sm border border-gray-100">
                    <p className="text-[8px] font-black text-gray-400 uppercase mb-1">Party A</p>
                    <p className="text-xs font-bold text-gray-900">{user?.division?.name}</p>
                  </div>
                  <div className="p-4 bg-white rounded-2xl shadow-sm border border-gray-100">
                    <p className="text-[8px] font-black text-gray-400 uppercase mb-1">Party B</p>
                    <p className="text-xs font-bold text-gray-900">{formData.client_legal_name || '...'}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {clauses?.filter(c => c.is_default || formData.custom_clause_ids.includes(c.id)).map((c, i) => (
                      <span key={c.id} className="px-3 py-1 bg-white border border-gray-100 rounded-lg text-[9px] font-bold text-gray-500">
