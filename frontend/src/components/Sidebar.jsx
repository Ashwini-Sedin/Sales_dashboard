...
const Sidebar = ({ isOpen, toggleSidebar }) => {
    ...
    return (
        <aside 
            className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-df-sidebar border-r border-gray-200 dark:border-df-border text-gray-800 dark:text-white transform transition-colors transition-transform duration-300 ease-in-out overflow-y-auto scrollbar-hide ${
                isOpen ? 'translate-x-0' : '-translate-x-full'
            } lg:translate-x-0 lg:static lg:inset-0`}
        >
            <div className="flex items-center justify-between h-20 px-6 border-b border-gray-200 dark:border-df-border">
                            ...
                            <div className="space-y-0.5">
                                {group.items.map((item) => (
                                    <NavLink
                                        ...
                                        to={item.path}
                                        className={({ isActive }) => 
                                            `flex items-center px-6 py-2.5 text-sm font-medium transition-all duration-200 group ${
                                                isActive 
