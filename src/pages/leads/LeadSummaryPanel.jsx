...
const LeadSummaryPanel = ({ lead, setActiveTab }) => {
  ...
  const getSourceIcon = (source) => {
    switch (source?.toLowerCase()) {
      case 'website': return <MdIcons.MdOutlineLanguage className="w-4 h-4 text-slate-500 dark:text-df-text" />;
      case 'referral': return <MdIcons.MdPerson className="w-4 h-4 text-slate-500 dark:text-df-text" />;
      default: return <MdIcons.MdBusinessCenter className="w-4 h-4 text-slate-500 dark:text-df-text" />;
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-df-card rounded-xl shadow-sm border border-slate-200 dark:border-df-border overflow-hidden mb-6 p-6 md:p-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-df-border text-slate-600 dark:text-gray-400">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <button 
            onClick={() => setActiveTab && setActiveTab('emails')}
            className="flex-1 lg:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-600 bg-transparent hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg text-sm font-semibold text-slate-700 dark:text-gray-300 transition-colors shadow-sm"
          >
            <MdIcons.MdOutlineEmail className="w-4 h-4 text-slate-500 dark:text-gray-400" />
            Email
          </button>
          <button 
          ...
          {STAGES.map((stage, idx) => {
            ...
            return (
              <div
                ...
                className={`flex-1 py-3 px-2 text-center text-sm font-semibold border-r border-slate-100 dark:border-slate-800 last:border-r-0 cursor-pointer transition-all duration-200 ${
                  isActive 
                    ? "text-[#0ebf99] bg-[#e5faef]/30 hover:bg-[#e5faef]/50" 
                    : "text-slate-400 dark:text-gray-500 hover:text-slate-600 dark:text-gray-400 hover:bg-slate-50 dark:bg-slate-800/50"
                }`}
              >
                {stage.label}
              </div>
