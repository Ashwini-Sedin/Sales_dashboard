...
const CreateLeadModal = ({ isOpen, onClose }) => {
  ...
  const globalRoles = ['admin', 'super_admin', 'ceo'];
  const canChooseDivision = globalRoles.includes(user?.role);
  const canCreateLead = !['legal', 'viewer'].includes(user?.role);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      .then(([divRes, userRes]) => {
        ...
        if (!canChooseDivision && user?.division_id) {
          setValue('division', user.division_id);
        }
      })
      .catch(console.error);
    }
  }, [isOpen, canChooseDivision, setValue, user?.division_id]);
  ...
  const onSubmit = (data) => {
    ...
    const payload = {
      ...
      division_id: canChooseDivision ? data.division : user?.division_id
    };
      ...
      <div className="bg-white dark:bg-df-card rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <select 
              {...register('division', { required: canChooseDivision ? 'Division is required' : false })} 
              disabled={!canChooseDivision}
              className="w-full px-3 py-2 bg-gray-50 dark:bg-[#10151b] border border-gray-200 dark:border-df-border rounded-lg focus:outline-none focus:ring-1 focus:ring-df-accent text-sm"
            >
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            <button 
              type="submit" 
              disabled={createLeadMutation.isPending || !canCreateLead}
