...
const QuickSalesForm = ({ leadId, lead, onClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const endpoint = format === 'pptx'
        ...
        : '/api/documents/quick-sales/generate';

      const response = await api.post(endpoint, payload);

      if (format === 'pptx' && response.data.document_id) {
        setTaskInfo({
          taskId: null,
          documentId: response.data.document_id,
          downloadUrl: response.data.download_url,
          sharepointUrl: response.data.sharepoint_url,
          syncComplete: true,
        });
      } else {
        setTaskInfo({
  ...
  if (taskInfo) {
    return (
      <GenerationProgressModal 
        ...
        leadId={leadId}
        onClose={onClose}
        syncComplete={taskInfo.syncComplete}
        initialDownloadUrl={taskInfo.downloadUrl}
        initialSharepointUrl={taskInfo.sharepointUrl}
      />
    );
  }
