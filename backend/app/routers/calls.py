...
async def get_call_recordings(
    ...
    if current_user.role not in [UserRole.admin, UserRole.super_admin, UserRole.ceo, UserRole.division_head] and recordings:
...
async def get_playback_url(
    ...
    # Division access check
    if current_user.role not in [UserRole.admin, UserRole.super_admin, UserRole.ceo, UserRole.division_head] and recording.division_id != current_user.division_id:
...
async def delete_call_recording(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    _: None = Depends(require_roles([UserRole.admin, UserRole.super_admin, UserRole.ceo, UserRole.division_head]))
):
    stmt = select(CallRecording).where(
        CallRecording.id == call_id,
        CallRecording.lead_id == lead_id
